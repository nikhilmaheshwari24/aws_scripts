...
for row in rows:
    if row[9] != 'Retain it' and row[3] == "us-east-1":
        snapshots_delete_it_north_virginia.append(row[0])
    elif row[9] != 'Retain it' and row[3] == "ap-southeast-1":
        snapshots_delete_it_singapore.append(row[0])
    elif row[9] != 'Retain it' and row[3] == "ap-south-1":
...
for region in regions:
    ...
    if region == "ap-south-1":
        for snapshot in snapshots_delete_it_mumbai:
            try:  
                delete_snapshot=client.delete_snapshot(SnapshotName=snapshot)
                print (delete_snapshot['Snapshot']['SnapshotName'], "-->", delete_snapshot['ResponseMetadata']['HTTPStatusCode'])
